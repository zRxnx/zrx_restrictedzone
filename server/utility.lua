...
local getRepoInformations = function()
    ...
    PerformHttpRequest('https://api.github.com/repos/zRxnx/zrx_restrictedzone/releases/latest', function(err, response, headers)
        if err == 200 then
            ...
            repoURL = 'https://github.com/zRxnx/zrx_restrictedzone'
        end
    end, 'GET')
...
CreateThread(function()
    if curResName ~= 'zrx_restrictedzone' then
        resourceName = ('zrx_restrictedzone (%s)'):format(curResName)
    end

    while continueCheck do
        PerformHttpRequest('https://github.com/zRxnx/zrx_restrictedzone/releases/latest', checkVersion, 'GET')
        Wait(3600000)
    end
end)